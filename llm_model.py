...
def create_llm(model_name="gemini-2.0-flash", temperature=0, google_api_key=None):
    return ChatGoogleGenerativeAI(
        model=model_name, temperature=temperature, google_api_key=google_api_key
    )
...
def get_versatile_llm(google_api_key):
    return create_llm(
        LLMConfig.VERSATILE, temperature=0.5, google_api_key=google_api_key
    )


def get_creative_llm(google_api_key):
    return create_llm(
        LLMConfig.CREATIVE, temperature=1.0, google_api_key=google_api_key
    )
...
def get_gemma27b_llm(google_api_key):
    return create_llm(
        LLMConfig.GEMMA27B, temperature=0.5, google_api_key=google_api_key
    )
